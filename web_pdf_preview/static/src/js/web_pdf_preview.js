...
var trigger_download = function(session, response, c, action, options) {
    ...
    if( /Android|webOS|iPhone|iPad|iPod|BlackBerry/i.test(navigator.userAgent) ) {
                    //url = "my.bluetoothprint.scheme://" + url;
    }
    //alert(url);
    ...
    if(action.context && action.context.direct_print){
        sleep(2000).then(() => {
             w.print();
        });
    }
    ...
    ir_actions_report_xml: function(action, options) {
        ...
        if ('report_type' in action && (action.report_type == 'qweb-html' || action.report_type == 'qweb-pdf' || action.report_type == 'controller')) {
            ...
            if (!('data' in action) || !(action.data)) {
                if ('original_ids' in action.context) {
                    report_url += "/" + action.context.original_ids.join(',') + "?enable_editor=1";
                }else{
